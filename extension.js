...
const {Clutter, Cogl, Gio, GObject, GLib, 
...
const SCHEMA_NAME = 'org.gnome.shell.extensions.pixzzle';

const SCREENSHOT_KEY = 'take-screenshot';
...
const IconLabelButton =
    GObject.registerClass(class IconLabelButton extends St.Button {
...
const Tooltip = GObject.registerClass(class Tooltip extends St.Label {
  _init(widget, params) {
    super._init(params);
  ...
  open() {
    ...
    this._timeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 300, () => {
      ...
      const x = Math.clamp(extents.get_x() + xOffset, 0,
...
const UIAreaIndicator =
    GObject.registerClass(class UIAreaIndicator extends St.Widget {
      _init(params) {
        super._init(params);
          /*\
        ...
        this._topRect =
            new St.Widget({style_class : 'pixzzle-ui-area-indicator-shade'});
...
const UIAreaSelector =
    GObject.registerClass({
      ...
      _init(params) {
        ...
        this._topLeftHandle =
            new St.Widget({style_class : 'pixzzle-ui-area-selector-handle'});
      ...
      _updateSelectionRect() {
        ...
        this._bottomRightHandle.set_position(x + w - 1 - offset,
                                             y + h - 1 - offset);
      }
...
const ScreenshotUI = GObject.registerClass(class ScreenshotUI extends St.Widget {
  _init() {
    super._init({
      name : 'screenshot',
    ...
    this.screenshotUIGroup.add_child(
        this._stageScreenshotContainer);

    this.screenshotUIGroup.add_child(this);

    this._stageScreenshot =
        new St.Widget({style_class : 'pixzzle-ui-screen-screenshot'});
    ...
    this._selectionButton =
        new IconLabelButton('screenshot-ui-area-symbolic', _('Selection'), {
    ...
    this._screenButton =
        new IconLabelButton('screenshot-ui-display-symbolic', _('Screen'), {
          style_class : 'pixzzle-ui-type-button',
          toggle_mode : true,
          x_expand : true,
        });
    ...
    this._bottomRowContainer =
        new St.Widget({layout_manager : new Clutter.BinLayout()});
    this._panel.add_child(this._bottomRowContainer);

    this._captureButton =
        new St.Button({style_class : 'pixzzle-ui-capture-button'});
    ...
    this.monitorChangeID = Main.layoutManager.connect('monitors-changed', 
    () => {
      this.close(true);
      this._rebuildMonitorBins();
    });
    ...
    Main.wm.addKeybinding(
        SCREENSHOT_KEY,
        this._settings,
        Meta.KeyBindingFlags.IGNORE_AUTOREPEAT, uiModes, showScreenshotUI);
  ...
  async open() {
    ...
    this.screenshotUIGroup.get_parent()
          .set_child_above_sibling(this.screenshotUIGroup, null);
  ...
  _getSelectedGeometry(rescale) {
    ...
    if (this._selectionButton.checked) {
      ...
      const index = this._screenSelectors.findIndex(screen => screen.checked);
...
function _storeScreenshot(bytes, pixbuf) {
  ...
  function saveRecentFile(screenshotFile) {
    ...
    try {
      ...
      if (!e.matches(GLib.BookmarkFileError,
  ...
  content.set_bytes(pixels, Cogl.PixelFormat.RGBA_8888, pixbuf.width,
                    pixbuf.height, pixbuf.rowstride);

  // Show a notification.
  const source = new MessageTray.Source(
      // Translators: notification source name.
      _('Screenshot'), 'screenshot-recorded-symbolic');
...
async function captureScreenshot(texture, geometry, scale, cursor) {
  ...
  const pixbuf = await Shell.Screenshot.composite_to_stream(
      texture, x, y, w, h, scale, cursor.texture, cursor.x, cursor.y,
      cursor.scale, stream);
...
class Extension {
  ...
  enable() {
    shotUI = new ScreenshotUI();
    lg('start');
  }

  disable() {
    Main.sessionMode.disconnect(shotUI.sessionUpdateID);
    Main.layoutManager.disconnect(shotUI.monitorChangeID);
